...
- Name: workflow.spec.cjs >> Scenario 2 — Player joins an existing tournament >> 2c — player can add themselves if not on the list
- Location: tests\e2e\workflow.spec.cjs:115:3
...
> 118 |     await expect(playerPage.getByTestId('who-are-you-screen')).toBeVisible({ timeout: 15000 });
  ...
  213 |     await playerPage.reload();
  214 |     await playerPage.waitForTimeout(5000);
  215 |     await expect(playerPage.getByTestId('landing-page')).toBeVisible({ timeout: 15000 });
  216 |     await expect(playerPage.getByTestId('app-header')).not.toBeVisible();
  217 |   });
  218 | });
```